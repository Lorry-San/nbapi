...
func OpenaiTTSHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) *dto.Usage {
	...
	if info.IsStream {
		...
		if durationErr != nil {
			...
			// Duration comes from upstream audio metadata and must not overflow token accounting.
			completionTokens := common.QuotaFromFloatRound(math.Ceil(duration) / 60.0 * 1000)
