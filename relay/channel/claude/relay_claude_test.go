...
func TestRequestOpenAI2ClaudeMessage_DefaultsInvalidToolSchemaType(t *testing.T) {
	request := dto.GeneralOpenAIRequest{
		Model: "claude-3-5-sonnet",
		Messages: []dto.Message{
			{
...
func TestRequestOpenAI2ClaudeMessage_ConvertsLegacyFunctions(t *testing.T) {
	request := dto.GeneralOpenAIRequest{
		Model: "claude-3-5-sonnet",
		Messages: []dto.Message{
			{
		...
		Functions: []byte(`[{"name":"lookup","description":"lookup docs","parameters":{"type":"object","properties":{"query":{"type":"string"}},"required":["query"]}}]`),
	}
	...
	require.Equal(t, "lookup docs", tool.Description)
	require.Equal(t, "object", tool.InputSchema["type"])
	require.NotNil(t, tool.InputSchema["properties"])
	require.Equal(t, []any{"query"}, tool.InputSchema["required"])
}
