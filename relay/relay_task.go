...
import (
	...
	"github.com/Lorry-San/nbapi/relay/channel/task/taskcommon"
...
func ResolveOriginTask(c *gin.Context, info *relaycommon.RelayInfo) *dto.TaskError {
	...
	if originTask.ChannelId != info.ChannelId {
		key, _, nbapiError := ch.GetNextEnabledKey()
		if nbapiError != nil {
			return service.TaskErrorWrapper(nbapiError, "channel_no_available_key", nbapiError.StatusCode)
		}
...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if !common.StringsContains(constant.TaskPricePatches, modelName) {
		for _, ra := range info.PriceData.OtherRatios {
			if ra != 1.0 {
				info.PriceData.Quota = common.QuotaFromFloatTrunc(float64(info.PriceData.Quota) * ra)
			}
		}
	}
	...
	c.Header("X-NBAPI-Other-Ratios", string(ratiosJSON))
...
func recalcQuotaFromRatios(info *relaycommon.RelayInfo, ratios map[string]float64) int {
	...
	for _, ra := range info.PriceData.OtherRatios {
		if ra != 1.0 && ra > 0 {
			baseQuota = common.QuotaFromFloatTrunc(float64(baseQuota) / ra)
		}
	}
	// 应用新的 ratios
	...
	return common.QuotaFromFloatTrunc(result)
}
