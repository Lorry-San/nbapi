...
func updateVideoSingleTask(ctx context.Context, adaptor channel.TaskAdaptor, channel *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	// try parse as NBAPI response format
	var responseItems dto.TaskResponse[model.Task]
	if err = common.Unmarshal(responseBody, &responseItems); err == nil && responseItems.IsSuccess() {
		logger.LogDebug(ctx, "UpdateVideoSingleTask parsed as NBAPI response format: %+v", responseItems)
				...
				if modelName, ok := taskData["model"].(string); ok && modelName != "" {
					...
					if hasRatioSetting && modelRatio > 0 {
						...
						if group != "" {
							...
							actualQuota := common.QuotaFromFloatTrunc(float64(taskResult.TotalTokens) * modelRatio * finalGroupRatio)
