...
export const channelFormSchema = z
  .object({
    ...
    openai_api_path: z.string().optional(), // OpenAI-compatible API path
...
export function transformChannelToFormDefaults(
    ...
    try {
      ...
      extraSettings = {
        ...
        force_responses_to_chat_completions:
          parsed.force_responses_to_chat_completions || false,
  ...
  if (channel.settings) {
    try {
      ...
      openAIAPIPath = parsed.openai_api_path || ''
...
function buildSettingJSON(formData: ChannelFormValues): string {
  const settingObj = {
    ...
    force_responses_to_chat_completions:
      formData.force_responses_to_chat_completions || false,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if (formData.type === 1) {
    const openaiAPIPath = formData.openai_api_path?.trim()
    if (openaiAPIPath) {
      settingsObj.openai_api_path = openaiAPIPath
    } else if ('openai_api_path' in settingsObj) {
      delete settingsObj.openai_api_path
    }
    ...
    if ('openai_api_path' in settingsObj) delete settingsObj.openai_api_path
