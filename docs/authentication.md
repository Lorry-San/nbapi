...
- 管理员可在“系统设置 → 安全设置 → 可信代理”中运行时关闭代理头信任。关闭后新请求会立即只使用 TCP 直连地址，并覆盖 `TRUSTED_PROXIES` 的启用效果；重新开启后继续按环境变量中的可信代理列表解析。
...
- 未配置 `TRUSTED_PROXIES` 时会兼容信任回环、常见私网代理和 Cloudflare 官方代理网段；使用其他公网负载均衡器、`100.64.0.0/10`、链路本地地址或自定义 CNI 网段的部署仍需显式配置。需要运行时严格忽略所有转发头时关闭后台“可信代理”开关，需要启动级严格模式时设置为 `none`。
