...
- 未配置、空字符串或纯空白的 `TRUSTED_PROXIES` 默认信任回环、RFC1918、IPv6 ULA 和 [Cloudflare 官方代理网段](https://www.cloudflare.com/ips/)，并输出启动告警。该默认值覆盖同机 Nginx、Docker Compose、常见内网反代和 Cloudflare；其他公网直连地址不在列表中，其伪造的 `X-Forwarded-For` 会被忽略。
...
- 管理员可在“系统设置 → 安全设置 → 可信代理”中运行时关闭可信代理来源校验。开启时仅接受 `TRUSTED_PROXIES` 白名单内代理提交的客户端 IP 请求头；关闭后会跳过来源校验，直接信任 `CF-Connecting-IP`、`True-Client-IP`、`X-Forwarded-For` 与 `X-Real-IP`，没有有效代理头时才回退到 TCP 直连地址。配置立即作用于新请求，无需重启。

校验开启时，Gin 只在请求的直连来源属于可信代理时解析客户端 IP 请求头，并从转发链右侧向左寻找首个非可信地址。因此常见 Nginx `$proxy_add_x_forwarded_for` 链中的公网客户端地址会阻止更左侧的伪造前缀生效。校验关闭时 NBAPI 会直接采用上述代理头，`X-Forwarded-For` 取最左侧首个有效 IP；此模式下能够直连 NBAPI 的客户端可以伪造来源 IP，因此只应在应用端口无法绕过反向代理访问，或全部调用方均受控时使用。
...
- 未配置 `TRUSTED_PROXIES` 时会兼容信任回环、常见私网代理和 Cloudflare 官方代理网段；使用其他公网负载均衡器、`100.64.0.0/10`、链路本地地址或自定义 CNI 网段的部署仍需显式配置。若部署已阻止绕过反向代理直连，可关闭后台“可信代理”校验开关以直接接受转发头；若要严格忽略所有转发头，应保持网页校验开启并设置 `TRUSTED_PROXIES=none`。
