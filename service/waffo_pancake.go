...
// are exposed separately for self-service flows driven from nbapi's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("nbapi-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "nbapi-store"
	defaultWaffoPancakeProductName = "nbapi-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because nbapi has no renewal-
// event handling; Pancake auto-renewing without nbapi extending user
